<?php
	...
	*/
function makeReflection($options) {
	...
	{
		$source_image = $options['img'];
		$source_image = str_replace('://','',$source_image);
		$source_image = $_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . "XBMC".DIRECTORY_SEPARATOR . $source_image;
        
        $cache_dir = $_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . "XBMC". DIRECTORY_SEPARATOR ."img";
        $cache_dir_sub = "reflectionsCache";
        $cache_base = basename($source_image);
        //$cache_file = 'refl_' . md5($_SERVER['REQUEST_URI']) . '_' . $cache_base;
        $cache_file = 'reflection_' . $cache_base;
        $cache_path = $cache_dir . DIRECTORY_SEPARATOR . $cache_dir_sub . DIRECTORY_SEPARATOR . $cache_file;
        
        $cached = false;
        if (is_file($source_image)){
			if (file_exists($cache_path) && filemtime($cache_path) >= filemtime($source_image)){
				print("<!-- using chached file-->");
				$cached = true;
			}else{
	...
	if (isset($options['fade_start']))
	{
		if (strpos($options['fade_start'], '%') !== false)
		{
			$alpha_start = str_replace('%', '', $options['fade_start']);
			$alpha_start = (int) (127 * $alpha_start / 100);
		}
		else
		{
			$alpha_start = (int) $options['fade_start'];
	...
	if (isset($options['fade_end']))
	{
		if (strpos($options['fade_end'], '%') !== false)
		{
			$alpha_end = str_replace('%', '', $options['fade_end']);
			$alpha_end = (int) (127 * $alpha_end / 100);
		}
		else
		{
			$alpha_end = (int) $options['fade_end'];
	...
	*/
    ...
    print("<!-- building finished and saved-->");
	imagedestroy($output);
    }
}
?>